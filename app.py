...
def get_player_name(playerID):
    url = "https://na1.api.riotgames.com/lol/summoner/v3/summoners/{}?api_key={}}".format(str(playerID), os.getenv('RIOT_API'))
    try:
...
def get_player_id(playerName):
    url = "https://na1.api.riotgames.com/lol/summoner/v3/summoners/by-name/{}?api_key={}}".format(playerName, os.getenv('RIOT_API'))
    try:
        data = urllib2.urlopen(url, timeout=12)
        statusCode = data.getcode()
        if statusCode == 200:
...
def get_top_5_champs(playerID):
    url = "https://na1.api.riotgames.com/lol/champion-mastery/v3/champion-masteries/by-summoner/{}?api_key={}}".format(playerID, os.getenv('RIOT_API'))
    try:
        data = urllib2.urlopen(url, timeout=12)
        statusCode = data.getcode()
        if statusCode == 200:
